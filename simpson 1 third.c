...
int main() {
    ...
    scanf("%d", &n);
    float  x[n + 1], f_values[n + 1];  // Arrays to store x values and corresponding f(x) values
float  integration , h ;
    printf("Enter the x values and corresponding function values (f(x)):\n");
    for (int i = 0; i <= n; i++) {
        printf("x[%d]: ", i);
        scanf("%f", &x[i]);
        printf("f(x[%d]): ", i);
    ...
    h = (x[n] - x[0]) / n;
   integration   = f_values[0] + f_values[n];  // Add the boundary values

    for (int i = 1; i < n; i++) {
        if (i % 2 == 0) {
            integration =   integration + 2 * f_values[i];  // Even indices: multiply by 2
        } else {
             integration =  integration+ 4 * f_values[i];  // Odd indices: multiply by 4
        }
    }
